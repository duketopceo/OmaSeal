...
**Per-secret AI policy (ai-manifest):** `~/.config/omaseal/ai-manifest.txt` is a robots.txt-style file governing agent (MCP) access per secret: `ALLOW` reads freely, `ASK` requires an unlocked session even in open mode, `DENY` refuses the call with code `manifest_denied`. Generate a starting manifest with `omaseal manifest init`, inspect it with `omaseal manifest`, and query a rule with `omaseal manifest check <service> <account>`. The JSON IPC channel is a trusted local surface for the panel and shell plugins — it bypasses agent trust modes and the manifest, so it should only be invoked by same-user programs.
