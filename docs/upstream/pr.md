...
This document describes the eventual upstream shape for OmaSeal. The current
plugin is the standalone prototype; the proposal below is the path toward
shipping it as an official Omarchy package.

## Goal

Make `omaseal` the default keyring facility in Omarchy, equivalent to macOS
...
- `org.freedesktop.Secret` compatibility is preserved. OmaSeal does not
  replace `gnome-keyring`; it sits as a user-namespace filter on top of it.
- `omarchy-shell` gains a first-party `omaseal` IPC target:
  - `get {service, account}`
...
- `omaseal` binary ships in `/usr/share/omarchy/bin` or `/usr/bin`.
- A `omarchy-omaseal` systemd user socket or D-Bus activation target is not
  needed; the binary is stateless and uses the Secret Service directly.
- Migration from the plugin to system package is `rm -rf
  ~/.config/omarchy/plugins/io.github.duketopceo.omaseal`; the data stays in
  `gnome-keyring`.
...
- Review `providers.go` to confirm `op` / `bw` secrets are only cached, never
  logged.
- Confirm `omaseal_set` MCP tool is only usable when the client is local and
...
- AUR `omaseal-git` and `omaseal-bin` PKGBUILDs.
- `omarchy` package can add `omaseal` as an optional dependency.
